...
if tab_idx == 7:
    ...
    if batch_mode == "多股批量":
        if "bt_factors" not in st.session_state:
            st.session_state.bt_factors = []

        current_factors = st.session_state.get("bt_factors", [])
        current_weights = [st.session_state.get(f"btw_{f}", 100 // max(len(current_factors), 1)) for f in current_factors]
        total_w = sum(current_weights) or 1
        bt_weights_batch = {f: w / total_w for f, w in zip(current_factors, current_weights)}
        bt_weight_list_batch = [bt_weights_batch.get(f, 0.0) for f in current_factors]
        ...
        if run_batch:
            ...
            target_regime_batch = regime_map_t.get(tune_regime, None) if "tune_regime" in dir() or True else None

            with st.spinner("批量回测运行中..."):
                all_results, stock_summaries = _run_batch_backtest(
                    stock_pool, start_str, end_str, index_code,
                    bt_window, bt_lookahead, bt_threshold, bt_topk, bt_algo,
                    current_factors, bt_weight_list_batch,
                    ensemble_mode, timing_filter, mom_filter,
                    None, walk_forward,  # target_regime=None for batch (global)
                )
    ...
    use_lgbm = st.checkbox("🤖 Optuna 联合优化权重", value=False, key="bt_lgbm",
                            help="用 Optuna 同时搜索因子权重 + 参数, 目标=信号段命中率, 替代手动滑块")
        ...
        if not bt_factors:
            ...
            if n < bt_window * 5:
                ...
                if total_days <= 0:
                    ...
                    macd_vals = df_factors["macd"].reindex(valid_bt.index).fillna(0).values
                    win = bt_window
                    ...
                    if use_lgbm and n_factors > 1:
                        ...
                        def optuna_objective(trial):
                            ...
                            results_train = eval_trial(
                             ...
                             low_vals=low_vals_t, regime_labels=regime_labels_t, target_regime=target_regime, mom_filter=mom_filter, macd_hist=macd_vals_t)
                        ...
                        for trial in all_trials:
                            ...
                            results_t = eval_trial(
                                ...
                                combined_corr, price_vals_t, n_tune,
                                tune_start, train_end, w_list,
                             ensemble_mode=ensemble_mode, timing_filter=timing_filter, vol_data=vol_data_t, vol_thresh=vol_thresh_t, low_vals=low_vals_t, regime_labels=regime_labels_t, target_regime=target_regime, mom_filter=mom_filter, macd_hist=macd_vals_t)
                        ...
                        if not valid_rows:
                            ...
                            for ti in range(top_n_test):
                                ...
                                if test_metrics:
                                    ...
                                    "CV均值±std": cv_str,
                                    "均收益%": test_metrics.get("均收益%", 0),
                                    "Sharpe": test_metrics.get("Sharpe", 0),
                                    "盈亏比": test_metrics.get("盈亏比", 0),
                                    "训练原始%": vrow["训练原始命中率%"],
                                        "验证原始%": vrow["验证原始命中率%"],
